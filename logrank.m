...
% figure
hold on
S1=stairs(t1,T1,'r','LineWidth',1); %Kaplan-Meier curve for treatment 1
if ~isempty(table12)
    S3=plot(xcg1,ycg1,'r+','LineWidth',1,'MarkerSize',5); %Censored data for treatment 1 (if there are)
else
    S3=[];
end

S2=stairs(t2,T2,'k','LineWidth',1); %Kaplan-Meier curve for treatment 2
if ~isempty(table22)
    S4=plot(xcg2,ycg2,'k+','LineWidth',1,'MarkerSize',5); %Censored data for treatment 2 (if there are)
else
    S4 = [];
end

S5 = plot(T1,ones(size(T1)),'k--');
hold off
...
% ylabel('Cum Progress Free Survival')%Estimated survival functions
% ylabel('Cum Overall Survival')%Estimated survival function
ylabel('Cum Progression-free Survival')%Estimated survival function
xlabel('Time since the start of immunotherapy(month)')
% xlabel('Time since diagnosis(month)')
% 
% if isempty(S3)&isempty(S4)
%     legend([S1 S2 S5],'<cutoff','>cutoff','P =')
% elseif isempty(S3) &~isempty(S4)
%     legend([S1 S2 S4 S5],'<cutoff','>cutoff','Censored','P =')
% elseif isempty(S4)&~isempty(S3)
%         legend([S1 S2 S3 S5],'low-risk','high-risk','Censored','P =')  
% elseif ~isempty(S4)&~isempty(S3)
%         legend([S1 S2 S3 S4 S5],'low-risk','high-risk','Censored','Censored','P =')
% 
% end
...
if isempty(S3)&isempty(S4)
    legend([S1 S2 S5],'>cutoff','<cutoff',strcat('P =',num2str(p)));
elseif isempty(S3) &~isempty(S4)
    legend([S1 S2 S4 S5],'>cutoff','<cutoff','Censored',strcat('P =',num2str(p)));
elseif isempty(S4)&~isempty(S3)
        legend([S1 S2 S3 S5],'>cutoff','<cutoff','Censored',strcat('P =',num2str(p)));  
elseif ~isempty(S4)&~isempty(S3)
        legend([S1 S2 S3 S4 S5],'>cutoff','<cutoff','Censored','Censored',strcat('P =',num2str(p)));
